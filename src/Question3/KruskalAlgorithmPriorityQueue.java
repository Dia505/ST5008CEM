...
/*
Approach: A priority queue has been created using min heap
A minimum heap arranges nodes in such a way that the root node has smaller value than of its children
The heap property is maintained in the priority queue by comparing the root node with its left and right child

Kruskal algorithm is used to create a minimum spanning tree from a graph
In this algorithm, minimum heap has been implemented in a way that the heap property is described.
When creating a minimum spanning tree, first, the edges of the graph need to be sorted.
Heap sort has been used to sort the edges.
The heap sort has been implemented in a way that the edges are sorted in ascending order, which can be seen in the heapify function
*/
